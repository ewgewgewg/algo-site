...
Arrays can also transform or `map` their values into a new array. The `map` callback function, a function used as a method argument, itself contains value and index arguments for every array location. Arrays can `sort` their values in place. Explicitly comparing two arguments in a callback lets you `sort` numbers, and if the return of the callback is positive the second argument will return first, while if the return of the callback is negative the first argument will return first. A tie leaves original order. Otherwise the default is to sort even numbers as strings. Arrays can also `slice` to return only the content between certain indexes. Arrays can also test `every` value for some condition and return a boolean (`true` or `false`) depending on if every test is passed.
